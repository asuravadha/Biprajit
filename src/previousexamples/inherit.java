...
inherit(String data){
    super(data);
    privatedata();
    System.out.println(data+" from previousexamples.inherit class");
}

    public static void inheritconcept(){
        Main obj=new Main("hai");
        obj.protecteddata();  // as it is non static i cant access it directly
        publicdata(); //as it is static method i can access it directly from previousexamples.inherit class
    }
